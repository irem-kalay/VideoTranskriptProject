...
API_KEY = "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"
...
# === Sadece Orijinal Transkript Çıkaran Fonksiyon (Güncellenmiş) ===
...
# === Transkript ile Özetleme Fonksiyonu (Güncellenmiş) ===
...
def process_endpoint():
    ...
    # İşleme başlarken progress_data'yı sıfırla
    progress_data["status"] = "processing"
    progress_data["current_video"] = None
    progress_data["results"] = []

    # Arka planda işlemi başlat
    threading.Thread(target=summarize_transcript, args=(save_path, ai_model)).start()

    # İlk yanıt sadece sürecin başladığını bildiriyor
    return jsonify({
        "status": "ok",
        "message": "İşlem başlatıldı. /progress ile durumu kontrol edebilirsiniz."
    })
